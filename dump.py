...
def main():
    ...
    tmp_dir = '/home/callbarian/backup_vidsurveil/C3D/C3D-v1.0/examples/c3d_feature_extraction/dump/tmp'

    video_file = "/home/callbarian/backup_vidsurveil/C3D/C3D-v1.0/examples/c3d_feature_extraction/dump/Explosion008_x264.mp4"


    # where feature csv file will be saved --
    # where the video is (by default), or second argument
    c3d_feature_outdir="/home/callbarian/backup_vidsurveil/C3D/C3D-v1.0/examples/c3d_feature_extraction/dump"
    '''
    if len(sys.argv) > 2:
    ...
    for start_frame in start_frames:
        ...
        f_input.write("{} {} {}\n".format(frame_dir, int(start_frame), dummy_label))

        # write "output_prefix.txt" with one clip
        clip_id = os.path.join(
                tmp_dir,
                video_id + '_{0:06f}'.format(int(start_frame))
                )
